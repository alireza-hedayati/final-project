...
function Navbar({ setProfile, profile, setMyTours,myTours }) {
  return (
    <div className="w-8/10 pt-8 pb-1 border-gray-400 mx-auto border-b  lg:border-gray-200 lg:border-[1px] lg:w-[220px] lg:h-fit lg:rounded-xl lg:shadow-sm lg:p-0">
      <ul className="flex items-center justify-between w-9/10 mx-auto lg:flex-col  lg:w-full lg:divide-y-[1px] lg:divide-gray-200">
        <li
          onClick={() => {
            setProfile(true), setMyTours(false);
          }}
          className={`${
            profile
              ? "text-green-500"
        ...
        <li
          onClick={() => {
            setMyTours(true), setProfile(false);
          }}
          className={`${
            myTours
              ? "text-green-500"
        ...
        <li onClick={()=>{setMyTours(false),setProfile(false)}} className="cursor-pointer flex items-center gap-1 hover:opacity-70  lg:w-full lg:py-3 lg:hover:bg-green-100 lg:hover:rounded-b-xl lg:pr-2">
          <span>
            <GrTransaction />
          </span>
          <p>تراکنش ها</p>
